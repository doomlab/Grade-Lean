...
3)  Is the perceived fairness of the grading in the course a moderator
    of reliability in instructor evaluations?
4)  Does the variability in instructor fairness rating predict
...
Finally, we will add the standard deviation of the fairness ratings to
the equation from RQ 3 as a fixed effect. The variability in fairness
ratings will be calculated in the same way as the mean fairness, which
is only for the instructor and semester time difference evaluations that
were used to calculate the reliability estimate. This variable
...
$R^2$ =
`r apa_num(as.numeric(r.squaredGLMM(rq2.model)[1]), gt1 = FALSE)`. WILL
...
```{r rq3-nlme}
rq3.model <- lme(rel ~ semester_diff*avg_fair, 
                 data = save_rel,
                 random = list(~1|instructor_code.x), 
                 method = "ML", 
                 na.action = "na.omit",
...
`r apa_num(as.numeric(r.squaredGLMM(rq3.model)[1]), gt1 = FALSE)`. WILL
INTERPRET THIS VALUE, RUN SIMPLE SLOPES IF SIGNIFICANT.

## RQ 4

```{r rq4-nlme}
rq4.model <- lme(rel ~ semester_diff*avg_fair + std_fair, 
                 data = save_rel,
                 random = list(~1|instructor_code.x), 
                 method = "ML", 
                 na.action = "na.omit",
...
The confidence interval for the addition of the variability of fairness
DID/DID NOT cross zero, *b* = `r apa_num(coef.rq4[5,1])`, 95% CI
[`r apa_num(ci.rq4$fixed[5, 1])`, `r apa_num(ci.rq4$fixed[5, 3])`],
$R^2$ =
`r apa_num(as.numeric(r.squaredGLMM(rq4.model)[1]), gt1 = FALSE)`. WILL
...
variability in fairness over time].
