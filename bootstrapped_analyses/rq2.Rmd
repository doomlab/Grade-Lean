...
for (i in 1:num_boots){
  ...
  save_results[[i]] <- lme(rel ~ semester_diff, 
                   data = DF_boot,
  ...
  save_coef[[i]] <- summary(save_results[[i]])$tTable[2, 1]
  save_ciL[[i]] <- intervals(save_results[[i]])$fixed[2,1]
  save_ciH[[i]] <- intervals(save_results[[i]])$fixed[2,3]
...
reliability did not cross zero, *b* = `r apa_num(coef_boot, digits = 3)`,
95% CI [`r apa_num(ciL_boot, digits = 3)`, `r apa_num(ciH_boot, digits = 3)`],
$R^2$ =
`r suppressWarnings(apa_num(R2_boot, gt1 = FALSE))`. The coefficient, while small, represents a small effect of time on the reliability of instructor ratings. 